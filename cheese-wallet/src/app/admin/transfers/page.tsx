...
const COLS     = ['User / Reference', 'Bank & Account', 'Amount NGN', 'Amount USDC', 'Status', 'Failure Reason', 'Date'];
const COL_GRID = '1.6fr 1.8fr 110px 110px 100px 1.8fr 100px';
    ...
    <div style={{ height: '100%', overflowY: 'auto', padding: '26px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        ...
        <div style={{ fontSize: 12, color: c.textDim, marginTop: 4 }}>
          Monitor NGN bank transfer requests — see failure reasons in real time
        </div>
      </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading && transfers.length === 0 ? (
            <div style={{ padding: '60px 22px', textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: c.textDim }}>Loading transfers…</div>
            </div>
          ) : transfers.length === 0 ? (
            <div style={{ padding: '60px 22px', textAlign: 'center' }}>
              <div style={{ fontSize: 14, color: c.textMid, fontWeight: 500, marginBottom: 6 }}>No transfers found</div>
        ...
        <span style={{ fontSize: 12, color: c.textDim }}>
          {total === 0 ? 'No transfers' : (
            <>
              ...
              <span style={{ color: c.text, fontWeight: 500 }}>{n(total)}</span> transfers
            </>
          )}
        </span>
