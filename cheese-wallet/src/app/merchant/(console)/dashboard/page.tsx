...
export default function MerchantDashboardPage() {
  ...
  return (
    <div className="min-w-0 space-y-6 p-4 sm:space-y-8 sm:p-6 md:p-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold tracking-[-0.03em] text-[color:var(--merchant-text)]">
          Welcome back
        </h1>
        <p className="mt-1 break-words text-sm text-[color:var(--merchant-muted)]">
        ...
        {isLoading ? (
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1 space-y-1.5">
                  <div className="h-3 w-40 max-w-full rounded bg-[color:var(--merchant-border)] animate-pulse" />
                  <div className="h-3 w-64 max-w-full rounded bg-[color:var(--merchant-border)] animate-pulse" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="divide-y divide-[color:var(--merchant-border)]">
            {data.recentActivity.map((item) => (
              <div key={item.id} className="grid grid-cols-[2rem_minmax(0,1fr)] items-start gap-x-3 gap-y-1 py-3 first:pt-0 last:pb-0 sm:grid-cols-[2rem_minmax(0,1fr)_auto]">
                <ActivityIcon type={item.type} />
                <div className="flex-1 min-w-0">
                  <p className="break-words text-sm font-medium text-[color:var(--merchant-text)]">
                    {item.title}
                  </p>
                  <p className="mt-0.5 break-words text-xs text-[color:var(--merchant-muted)]">
                    {item.note}
                  </p>
                </div>
                <span className="col-start-2 text-xs text-[color:var(--merchant-muted)] sm:col-start-3 sm:mt-0.5">
                  {timeAgo(item.at)}
                </span>
              </div>
            ))}
      ...
      {!isLoading && data && (
        <div className="flex items-center gap-2 text-xs text-[color:var(--merchant-muted)]">
          <svg viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 shrink-0 text-emerald-400">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
