...
export class DojahClient implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    if (appId && secretKey) {
      ...
      this.logger.warn(
        'Dojah not configured — KYC features disabled (set DOJAH_APP_ID and DOJAH_SECRET_KEY)',
      );
    }
  }
  ...
  private async parseResponse<T>(res: Response): Promise<T> {
    ...
    try {
      ...
      throw new Error(
        `Dojah returned non-JSON (${res.status}): ${text.slice(0, 200)}`,
      );
    }
