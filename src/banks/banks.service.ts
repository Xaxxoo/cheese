...
export class BanksService {
  ...
  async bankTransfer(userId: string, dto: BankTransferDto) {
    ...
    const { usdc: usdcBalance } = await this.blockchainService.getStellarBalance(
    ...
    try {
      ...
      await this.transferRepo.update(
        { id: transfer.id },
        { status: BankTransferStatus.FAILED, failureReason: (err as Error).message },
      );
      await this.txService.update(tx.id, {
        status:        TxStatus.FAILED,
        failureReason: (err as Error).message,
      });
      throw new BadRequestException(`USDC debit failed: ${(err as Error).message}`);
    }
    ...
    return {
      reference,
      status:      'processing',
      message:     'USDC deducted. NGN payout is being processed. You will receive a confirmation once the transfer settles.',
      amountNgn:   dto.amountNgn,
      amountUsdc,
      rateApplied: rate.effectiveRate,
  ...
  async processWebhook(dto: BankWebhookDto) {
    ...
    if (!transfer) {
      throw new NotFoundException(`No transfer found with reference: ${dto.reference}`);
    }
    ...
    switch (dto.event) {
      ...
      case 'transfer.success': {
        ...
        return { processed: true, reference: dto.reference, status: 'completed' };
      }
      ...
      case 'transfer.reversed': {
        ...
        const user = await this.userRepo.findOne({ where: { id: transfer.userId } });
        ...
        await this.txService.updateByReference(dto.reference, {
          status:        newStatus === BankTransferStatus.REVERSED ? TxStatus.REVERSED : TxStatus.FAILED,
          failureReason: dto.failureReason ?? dto.event,
        });
